...
use gtk::{
    ...
    FileChooserDialog, FileFilter, Label, Orientation, ResponseType, ScrolledWindow, TextView,
};
...
pub fn build_ui(app: &gtk::Application) {
...

    {
        ...
        choose_button.connect_clicked(move |_| {
            ...
            for pattern in [
                "*.mp4", "*.mov", "*.avi", "*.webm", "*.png", "*.jpg", "*.jpeg", "*.webp", "*.heic",
                "*.avif",
            ] {
                ...
                if response == ResponseType::Accept {
                    if let Some(path) = dialog.file().and_then(|f| f.path()) {
                        ...
                        *input_path.borrow_mut() = Some(path.clone());
                        ...
                        let video_outputs = vec![("mp4", "MP4 (H.264)")];
...

    {
        ...
        convert_button.connect_clicked(move |_| {
            let Some(path) = input_path.borrow().clone() else {
                error_buffer.set_text("❌ Please select a file before converting.");
                return;
            };

            let selected_format = format_selector.active_text().map(|f| f.to_string()).unwrap_or_default();
            match ffmpeg::convert_file(&path, &selected_format) {
                Ok(out_path) => {
                    let msg = format!("✅ Saved to: {}", out_path);
                    error_buffer.set_text(&msg);
                }
                Err((status, message)) => {
                    let msg = format!("❌ {}\n{}", status, message);
                    error_buffer.set_text(&msg);
